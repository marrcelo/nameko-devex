...
class TestDeleteProduct(object):
    ...
    def test_product_not_found(self, gateway_service, web_session):
        ...
        # call the gateway service to delete order #1
...
class TestGetOrder(object):
    ...
    def test_can_get_order(self, gateway_service, web_session):
        ...
        gateway_service.orders_rpc.get.return_value = {
            'id': 1,
            'order_details': [
                {
                    'id': 1,
        ...
        assert [call(1)] == gateway_service.orders_rpc.get.call_args_list
        assert [call(['the_odyssey', 'the_enigma'])
                ] == gateway_service.products_rpc.list.call_args_list or [call(['the_enigma', 'the_odyssey'])
                                                                          ] == gateway_service.products_rpc.list.call_args_list

    def test_order_not_found(self, gateway_service, web_session):
        gateway_service.orders_rpc.get.side_effect = (
    ...
    def test_can_list_order(self, gateway_service, web_session):
        ...
        # call the gateway service to get order #1
        response = web_session.get('/orders?page=1&limit=5')
        print(response)
        print(response.json())
        assert response.status_code == 200
        ...
        # check dependencies called as expected
        assert [call(1)] == gateway_service.orders_rpc.list.call_args_list
        assert [call(['the_odyssey', 'the_enigma'])
                ] == gateway_service.products_rpc.list.call_args_list or [call(['the_enigma', 'the_odyssey'])
                                                                          ] == gateway_service.products_rpc.list.call_args_list
...
class TestCreateOrder(object):
    ...
    def test_can_create_order(self, gateway_service, web_session):
        ...
        gateway_service.orders_rpc.create.return_value = {
            'id': 11,
            'order_details': []
        }
        ...
        assert gateway_service.products_rpc.list.call_args_list == [
            call(['the_odyssey'])]
        assert gateway_service.orders_rpc.create.call_args_list == [
            call([
